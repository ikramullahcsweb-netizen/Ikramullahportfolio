...
// const ProjectCard = (
//    {title , main,button1,button2,button3,date,more,li,paragraph,li1,li2,li3,li4,li5,button4,button5})  => {
...
const ProjectCard = ({ title, main, button1, button2, button3, date, more, li, paragraph, li1, li2, li3, li4, li5, button5 }) => {
  const [showModel, setShowModel] = useState(false);

  return (
    <div className='w-full'>
      <div 
        className='w-full md:w-95 md:m-1 h-55 p-2 flex flex-col rounded-xl bg-white border-gray-300 border-1 font-sans hover:scale-103 hover:opacity-60 transition duration-800 cursor-pointer' 
        onClick={() => setShowModel(true)}
      >
        <h3 className='px-2 text-xl font-semibold text-gray-800 py-4 leading-normal'>{title}</h3>
        <p className='px-2 text-gray-700 leading-tight tracking-normal'>{main}</p>
        
        <div className='flex gap-2 md:gap-4 mt-2 p-2'>
          <div className='items-center justify-center text-center flex sm:gap-3 gap-1'>
            <button className="px-3 bg-gray-100 text-black rounded-4xl transition">{button1}</button>
            <button className='px-3 text-gray-950 bg-gray-100 rounded-4xl hover:opacity-85 duration-300 hover:scale-105'>{button2}</button>
            <button className='px-4 text-gray-950 bg-gray-100 rounded-4xl hover:opacity-85 duration-300 hover:scale-105'>{button3}</button>
          </div>
        </div>
      ...
      {showModel && (
        <div className='fixed inset-0 bg-black/30 z-50 flex justify-center items-center p-4 overflow-y-auto'>
          <div className='bg-white p-6 w-full max-w-3xl rounded-2xl relative max-h-[90vh] overflow-y-auto'>
            <div className='absolute top-4 right-4 cursor-pointer' onClick={() => setShowModel(false)}>
              <FiX size={19} />
            </div>
            ...
            <div className='flex md:flex-row flex-col gap-8'>
              ...
              <div className='flex flex-col gap-5'>
                ...
                <div className='flex flex-wrap gap-2'>
                  <button className="px-3 bg-gray-100 rounded-4xl">{button1}</button>
                  <button className='px-3 bg-gray-100 rounded-4xl'>{button2}</button>
                  <button className='px-4 bg-gray-100 rounded-4xl'>{button3}</button>
                  <button className='px-4 bg-gray-100 rounded-4xl'>{button5}</button>
                </div>
              </div>
            </div>
          </div>
